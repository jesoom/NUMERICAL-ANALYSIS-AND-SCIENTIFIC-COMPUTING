...
% ===='advection:centred finite difference'====
...
lambda=a*(dt/dx);
A3=ones(1,length(x));
A4=-(lambda/2)*ones(1,length(x)-1);
A5=(lambda/2)*ones(1,length(x)-1);
A_C=diag(A3)+diag(A4,1)+diag(A5,-1);
A_C(1,end)=(lambda/2);
A_C(end,1)=-(lambda)/2;
c_old=y0;
for i=1:length(t)-1
    c_new=A_EE*c_old+A_C*c_old;
    c_new=c_new-c_old;
    c_old=c_new;
end
